...
-- Use to sum rows of output Product, CoProduct and Food Rescue
...
BEGIN
	...
	SELECT @resultValuePerMass = ISNULL(INCOME, 0) / WEIGHT
	FROM DXDIR_OUTPUT
...
BEGIN
	...
	IF (@sDESTINATION_CODE = 'PRODUCT')
		SET @dcMargin = 1;
	ELSE IF (@sDESTINATION_CODE IN ('COPRODUCT', 'COPRODUCT_2', 'FOOD_RESCUE'))
	  BEGIN

        SET @productValuePerMass = dbo.DXDIR_PK_CALCULATIONS$GetValuePerMass(@sASSESSMENT_CODE, @dcASSESSMENT_LC_STAGE_ID, 'PRODUCT'); 
		...
		ELSE
		  BEGIN
			SET @valuePerMass = dbo.DXDIR_PK_CALCULATIONS$GetValuePerMass(@sASSESSMENT_CODE, @dcASSESSMENT_LC_STAGE_ID, @sDESTINATION_CODE);
...
BEGIN
	DECLARE @result NUMERIC(25, 14) = 0,
			@productValuePerMass NUMERIC(25, 12) = dbo.DXDIR_PK_CALCULATIONS$GetValuePerMass(@sASSESSMENT_CODE, @dcASSESSMENT_LC_STAGE_ID, 'PRODUCT');
...
(
	...
	@dcCoProductSecondMargin NUMERIC(26, 12),
	@dcCoProductSecondWeight NUMERIC(25, 3),
...
BEGIN
	...
	RETURN (@dcCost * @dcFactor) - 
	       ((@dcCoProductWeight / @dcTotalMass) * @dcCost * @dcCoProductMargin) -
		   ((@dcCoProductSecondWeight / @dcTotalMass) * @dcCost * @dcCoProductSecondMargin) -
		   ((@dcFoodRescueWeight / @dcTotalMass) * @dcCost * @dcFoodRescueMargin) -
		   (@dcOutputRevenueFactor * @dcCost);
END
GO