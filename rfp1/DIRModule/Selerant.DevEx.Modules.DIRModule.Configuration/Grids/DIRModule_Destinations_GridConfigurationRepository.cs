...
{
	public class DIRModule_Destinations_GridConfigurationRepository : BaseConfigurationRepository<HtmlGridConfiguration>, ICoreHtmlGridConfigurationRepository
	{
		public string GridName => GridNames.DESTINATIONS;
		...
		private HtmlGridConfiguration Version_39(HtmlGridConfiguration config)
		{
			...
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("ANIMAL_FEED");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("BIOMASS_MATERIAL");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("CODIGESTION_ANAEROBIC");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("COMPOSTING");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("COMBUSTION");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("LAND_APP");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("LANDFILL");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("NOT_HARVESTED");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("REFUSE_DISCARD");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("SEWER");
			config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumn("ENVIRONMENT_LOSS");
		...
		private HtmlGridConfiguration Version_3_10_1(HtmlGridConfiguration config)
		{
            ...
            config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumnAfter("RECYCLING", "LAND_APP");
            config.HtmlGridUIs.GetOrCreateHtmlGridUI().WithColumnAfter("INCIN_WITH_EN_RECOVER", "RECYCLING");
