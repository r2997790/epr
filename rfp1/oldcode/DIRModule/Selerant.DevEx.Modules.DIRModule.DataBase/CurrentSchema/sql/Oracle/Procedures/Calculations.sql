CREATE OR REPLACE PACKAGE DXDIR_PK_CALCULATIONS AS
	...
	(
		...
		dcCoProductSecondMargin NUMBER,
		dcCoProductSecondWeight NUMBER,
...
CREATE OR REPLACE PACKAGE BODY DXDIR_PK_CALCULATIONS AS 
	...
	BEGIN
		...
		THEN
			dcMargin := 1;
		ELSIF (sDESTINATION_CODE IN ('COPRODUCT', 'COPRODUCT_2', 'FOOD_RESCUE'))
		THEN
			-- just primary product is involved in calculations
            productValuePerMass := GetValuePerMass(sASSESSMENT_CODE, dcASSESSMENT_LC_STAGE_ID, 'PRODUCT'); 
        
			IF (productValuePerMass = 0)
			THEN
				dcMargin := 0;
			ELSE
                valuePerMass := GetValuePerMass(sASSESSMENT_CODE, dcASSESSMENT_LC_STAGE_ID, sDESTINATION_CODE);
